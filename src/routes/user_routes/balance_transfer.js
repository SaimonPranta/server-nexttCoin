    ...
    try {
        ...
        if (id && amount && phoneNumber) {
            ...
            if (receiverUserVarifiy.length > 0) {
                const receiverUser = await user_collection.findOne({ phoneNumber: phoneNumber.toString() });
                ...
                if (receiverUser._id && provideUser._id) {
                    ...
                    if (providerfloorAmount >= floorAmount) {
                        const receiverUserUpdate = await user_collection.findOneAndUpdate({ phoneNumber: phoneNumber },
                            {
                                $set: {
