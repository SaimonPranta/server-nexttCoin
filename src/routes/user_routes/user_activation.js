        ...
        if (hostUser && hostUser.referNumber) {
            if (!hostUser.isActive) {
                if (floorBalance >= 50) {
                    ...
                    for (let i = 1; i <= 10; i++) {
                        if (hostUser && hostUser._id && hostUser.referNumber && i == 1) {
                            ...
                            if (document.length < 1) {
                                ...
                                if (currentUserForBalance && currentUserForBalance._id) {
                                    ...
                                    const refarUserChecker = await user_collection.find({ phoneNumber: currentUser.referNumber }).select({
                                        _id: 1,
                                        phoneNumber: 1
                                    })

                                    if (refarUserChecker.length > 0) {
                                        refUser = currentUser
                                    } else {
                                        refUser = {}
                    ...
                    res.status(500).json({ failed: "You Have Insufficient Balance, Please Invest Minimum 50TK & Try Again !" })
                }
            } else {
                res.status(500).json({ failed: "You Are Already Active !" })
            }
        } else {
